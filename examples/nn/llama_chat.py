...
from lib.models.nlp.llama import Transformer, convert_from_huggingface, fix_bf16
...
def fix_bf16(weights: Dict[Any, Tensor]):
    if getenv("SUPPORT_BF16", 1):
        ...
        return {k: v.cast(slope.float16) if v.dtype == slope.bfloat16 else v for k, v in weights.items()}
    # TODO: check if device supports bf16
    return {k: v.llvm_bf16_cast(slope.half).to(v.device) if v.dtype == slope.bfloat16 else v for k, v in weights.items()}
...
# **** helper functions ****
def concat_weights(models, device=slope.DEFAULT_DEVICE):
...
class AbsmaxQuantizedLinear:
    def __init__(self, in_features, out_features, bias=False):
        assert bias == False
        self.weight = slope.ones(out_features, in_features, dtype=slope.int8)
        self.scale = slope.ones(out_features, dtype=slope.half)

    def __call__(self, x):
        return x.dot(self.weight.cast(dtype=slope.half).T * self.scale)
    ...
    def quantize(tensors):
        ...
        for name, v in tensors.items():
            if "feed_forward" in name or ("attention.w") in name or name == "output.weight":
                ...
                int8_weight = (v.T / scale).T.cast(dtype=slope.int8)
