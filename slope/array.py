...
class Array:
    ...
    def __init__(
        ...
        if type(val) in (tuple, list):
            dtype = dtype or self.default_type
            self.val = np.array(val, dtype)
        elif type(val) is np.ndarray:
            dtype = val.dtype or self.default_type
    ...
    def flip(self, axis, *args):
        return ops.Flip.do(
            self,
            axis=[x if x >= 0 else x + len(self.shape) for x in utils.argfix(axis, *args)],
        )
    ...
    def shrink(self, arg: Tuple[Tuple[int, int], ...]):
        return (ops.Shrink.do(self, arg=arg)
    ...
    def concatenate(self, *args, dim=0):
        ...
        return reduce(self.__class__.__add__, [arg.slice(s) for arg, s in zip(catargs, slc)])
