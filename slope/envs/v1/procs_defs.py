...
def __getitem__(
    ...
    if (num_slices := len(count[int]) + len(count[slice]) + len(count[Tensor])) > len(
        self.shape
    ):
...
#   def slice(self, arg:Sequence[Optional[Tuple[int, int]]], value:float=0) :
    ...
    return (
        (
            ...
            * self.permute(*permarg)
            .shrink(tuple([*[(0, sh) for sh in idx.shape[1:-1]], (0, self.shape[dim])]))
            .unsqueeze(0)
        )
        .sum(-1)
