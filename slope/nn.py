...
class Optimizer:
  def __init__(self, params: Tuple[Tensor], lr: float):
    ...
    self.params: Tuple[Tensor] = dedup([x for x in params if x.requires_grad])
    self.buffers: Tuple[Tensor] = dedup([x for x in params if not x.requires_grad])   # buffers are still realized
...
class SGD(Optimizer):
  def __init__(self, params: Tuple[Tensor], lr=0.001, momentum=0, weight_decay=0.0, nesterov=False):
...
def AdamW(params: Tuple[Tensor], lr=0.001, b1=0.9, b2=0.999, eps=1e-8, wd=0.01): return LAMB(params, lr, b1, b2, eps, wd, adam=True)
def Adam(params: Tuple[Tensor], lr=0.001, b1=0.9, b2=0.999, eps=1e-8): return LAMB(params, lr, b1, b2, eps, 0.0, adam=True)
