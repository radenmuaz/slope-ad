...
def compute_fans(shape: Sequence, in_axis=-2, out_axis=-1,
                  batch_axis=()):
...
def normal(stddev=1e-2, dtype=None) -> Callable:
  def init(shape, dtype=dtype):
    return np.random.normal(shape, dtype) * stddev
  return init
...
def variance_scaling(scale, mode: str, distribution: str,
  ...
  def init(key, shape, dtype=dtype):
    ...
    if distribution == "normal":
      return np.random.normal(shape, dtype) * np.sqrt(variance)
    elif distribution == "uniform":
      return np.random.uniform(shape, dtype, -1) * np.sqrt(3 * variance)
    else:
...
def glorot_normal(in_axis: Union[int, Sequence[int]] = -2,
                  ...
                  batch_axis: Sequence[int] = (),
                  dtype=None) -> Callable:
  return variance_scaling(1.0, "fan_avg", "normal", in_axis=in_axis,
