...
def optimizer(
    ...
    def tree_opt_maker(*args, **kwargs):
        ...
        def tree_update(i, grad_tree, opt_state):
            ...
            grad_flat, tree2 = ad.tree_flatten(grad_tree)
            if tree2 != tree:
                msg = (
        ...
        def tree_get_params(opt_state):
            states_flat, tree, subtrees = opt_state
            states = utils.list_map(ad.tree_unflatten, subtrees, states_flat)
            params = utils.list_map(get_params, states)
            return ad.tree_unflatten(tree, params)
...
def unpack_optimizer_state(opt_state):
    ...
    subtrees = map(ad.tree_unflatten, subtree_defs, states_flat)
    sentinels = [JoinPoint(subtree) for subtree in subtrees]
    return ad.tree_unflatten(tree_def, sentinels)
...
def pack_optimizer_state(marked_pytree):
    ...
    """
    sentinels, tree_def = ad.tree_flatten(marked_pytree)
    assert all(isinstance(s, JoinPoint) for s in sentinels)
    subtrees = [s.subtree for s in sentinels]
    states_flat, subtree_defs = ad.unzip2(map(ad.tree_flatten, subtrees))
