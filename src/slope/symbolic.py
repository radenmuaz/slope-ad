...
class NumNode(Node):
    ...
    def __hash__(self):
        return hash(self.b)  # needed with __eq__ override
...
class MulNode(OpNode):
    ...
    def __mod__(self, b: Union[Node, int]):
        return Node.__mod__(self.a * (self.b % b), b)
...
class SumNode(RedNode):
    ...
    def __mod__(self, b: Union[Node, int]):
        ...
        for x in self.nodes:
            if x.__class__ in (NumNode, MulNode):
                new_nodes.append(x % b)  # might simplify
            else:
                new_nodes.append(x)
    ...
    def flat_components(self):
        return [y for x in self.nodes for y in (x.flat_components if isinstance(x, SumNode) else [x])]
...
render_python: Dict[Type, Callable] = {
    ...
    if isinstance(self.a, Variable) and isinstance(self.b, Variable) and self.a.expr and self.b.expr and self.b.expr < self.a.expr
